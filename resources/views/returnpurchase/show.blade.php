    ...
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <div class="mb-5">
            <ul class="m-0 p-0 list-none">
                ...
                <li class="inline-block relative text-sm text-primary-500 font-Inter ">
                    <a href="{{route('returnpurchase.index')}}">Return Purchase</a>
        ...
        <div class="lg:flex justify-between flex-wrap items-center mb-6">
            ...
            <div class="flex lg:justify-end items-center flex-wrap space-xy-5">
                <a href="{{route('returnpurchase.pdf',['id'=>$returnPurchase->id])}}" class="invocie-btn inline-flex btn btn-sm whitespace-nowrap space-x-1 cursor-pointer bg-white dark:bg-slate-800
                ...
                <button class="invocie-btn inline-flex btn btn-sm whitespace-nowrap space-x-1 cursor-pointer bg-white dark:bg-slate-800
                    ...
                    <span>Send returnPurchase</span>
                </button>
            </div>
        </div>
        <!-- returnPurchase Header Card -->
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-slate-50 dark:bg-slate-800">
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-sm text-slate-500 dark:text-slate-300 space-y-2">
                        <p class="font-medium">{{ $returnPurchase->purchaseOrder->outlet->name }}</p>
                        <p class="whitespace-pre-line">{{ $returnPurchase->purchaseOrder->outlet->address }}</p>
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                            </svg>
                            <span>{{ $returnPurchase->purchaseOrder->outlet->phone }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-sm text-slate-500 dark:text-slate-300 space-y-2">
                        <p class="font-medium">{{ $returnPurchase->purchaseOrder->supplier->name }}</p>
                        <p class="whitespace-pre-line">{{ $returnPurchase->purchaseOrder->supplier->address }}</p>
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                            </svg>
                            <span>{{ $returnPurchase->purchaseOrder->supplier->phone }}</span>
                        </div>
                    </div>
                </div>

                <!-- returnPurchase Details -->
                <div class="space-y-3">
                    <h2 class="text-xl font-medium text-slate-900 dark:text-slate-300">returnPurchase Details:</h2>
                    <div class="text-sm space-y-2">
                        <div>
                            <h4 class="text-xs uppercase font-medium text-slate-600 dark:text-slate-300">return purchase Number:</h4>
                            <p class="text-slate-500 dark:text-slate-300">{{ $returnPurchase->return_number }}</p>
                        </div>
                        <div>
                            <h4 class="text-xs uppercase font-medium text-slate-600 dark:text-slate-300">return purchase Date:</h4>
                            <p class="text-slate-500 dark:text-slate-300">{{ Carbon\Carbon::parse($returnPurchase->created_at)->translatedFormat('d F Y') }}</p>
                        </div>
        ...
        <!-- returnPurchase Products Table -->
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow mt-6">
            <div class="p-6">
                <h3 class="text-lg font-medium mb-4 text-slate-900 dark:text-slate-300">Return Purchase Products</h3>
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                            @foreach($returnPurchase->productReturnPurchases as $product)
                            <tr>
                            ...
                            <tr class="font-bold">
                                ...
                                <td class="px-4 py-3 text-right text-slate-900 dark:text-slate-300">
                                    {{ currency($returnPurchase->productReturnPurchases->sum('total')) }}
                                </td>
                            </tr>
                        </tbody>
                    </table>

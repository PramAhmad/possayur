...
<body>
        ...
        <table border="0">
            <tr>
                <td class="logo-container">
                    <img src="{{ public_path('upload/outlets/' . $invoice->salesOrder->outlet->logo) }}" alt="{{ $invoice->salesOrder->outlet->name }}">
                </td>
                <td class="outlet-header">
                    <h1>{{ $invoice->salesOrder->outlet->name }}</h1>
                    <p>{{ $invoice->salesOrder->outlet->address }}</p>
                    <p>Telepon: +62 851-9898-9744 | Email: info@gubugbuah.com</p>
                </td>
                <td class="details">
                    <p><strong>Tanggal:</strong> {{ $invoice->salesOrder->created_at->format('d M Y') }}</p>
                    <p><strong>No Invoice:</strong> INV-{{ $invoice->salesOrder->id }}</p>
                    <p><strong>Kepada Yth:</strong> {{ $invoice->salesOrder->customer->name }}</p>
                    <p><strong>Alamat:</strong> {{ $invoice->salesOrder->customer->address }}</p>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="footer">
        ...
        <p>Untuk pembayaran via transfer: BCA - 8832 901 902 a.n {{$invoice->salesOrder->outlet->name}}</p>
    </div>
</body>
</html>

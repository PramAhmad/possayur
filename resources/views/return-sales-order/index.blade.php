        ...
        <div class="card">
            <header class="card-header noborder">
                <div class="justify-end flex gap-3 items-center flex-wrap">
                    <!-- @can('returnsalesorder create')
                    <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2 !px-3" href="{{ route('returnsalesorder.create') }}">
                        <iconify-icon icon="ic:round-plus" class="text-lg mr-1"></iconify-icon>
                        {{ __('New Return Sales Order') }}
                    </a>
                    @endcan -->

                    <a class="btn inline-flex justify-center btn-dark rounded-[25px] items-center !p-2.5" href="{{ route('returnsalesorder.index') }}">
                        <iconify-icon icon="mdi:refresh" class="text-xl"></iconify-icon>
                    </a>
                    @can('returnsalesorder export')    
                     <a href="{{ route('returnsalesorder.export') }}" class="btn btn-success inline-flex justify-center btn-dark rounded-[25px] items-center !p-2.5">
                        ...
                        <div class="overflow-hidden">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                <tbody class="bg-white divide-y divide-slate-100 dark:bg-slate-800 dark:divide-slate-700">
                                    @forelse ($returnSalesOrder as $r)
                                    <tr>
                                        <td class="table-td"># {{ $r->return_number }}</td>
                                        <td class="table-td">{{ $r->salesOrder->reference_no ?? '-' }}</td>
                                        ...
                                        <td class="table-td">
                                            <div class="flex space-x-3 rtl:space-x-reverse">
                                                @can('returnsalesorder view')
                                                <a class="action-btn" href="{{ route('returnsalesorder.show', ['returnsalesorder' => $r]) }}">
                                                ...
                                                @can('returnsalesorder delete')
                                                <form id="deleteForm{{ $r->id }}" method="POST" action="{{ route('returnsalesorder.destroy', $r) }}">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            <x-table-footer :per-page-route-name="'returnsalesorder.index'" :data="$returnSalesOrder" />
                        </div>
                    </div>
                </div>
            </div>

<x-app-layout>
    ...
    <div class="mb-5">
        ...
        <div class="lg:flex justify-between flex-wrap items-center mb-6">
            <h4>Return Sales Order Penagihan
            </h4>
            <div class="flex lg:justify-end items-center flex-wrap space-xy-5">
                <a href="{{route('returnsalesorder.pdf',['id'=>$returnSalesOrder->id])}}" class="invocie-btn inline-flex btn btn-sm whitespace-nowrap space-x-1 cursor-pointer bg-white dark:bg-slate-800
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 p-6 bg-slate-50 dark:bg-slate-800">
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-sm text-slate-500 dark:text-slate-300 space-y-2">
                        <p class="font-medium">{{ $returnSalesOrder->outlet->name }}</p>
                        <p class="whitespace-pre-line">{{ $returnSalesOrder->outlet->address }}</p>
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                            </svg>
                            <span>{{ $returnSalesOrder->outlet->phone }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-sm text-slate-500 dark:text-slate-300 space-y-2">
                        <p class="font-medium">{{ $returnSalesOrder->salesorder->customer->name }}</p>
                        <p class="whitespace-pre-line">{{ $returnSalesOrder->salesorder->customer->address }}</p>
                        <div class="flex items-center gap-2">
                            <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                            </svg>
                            <span>{{ $returnSalesOrder->salesorder->customer->phone }}</span>
                        </div>
                    </div>
                </div>

                <!-- returnSalesOrder Details -->
                <div class="space-y-3">
                    <h2 class="text-xl font-medium text-slate-900 dark:text-slate-300">Return Sales Order Details:</h2>
                    <div class="text-sm space-y-2">
                        <div>
                            <h4 class="text-xs uppercase font-medium text-slate-600 dark:text-slate-300">returnSalesOrder Number:</h4>
                            <p class="text-slate-500 dark:text-slate-300">{{ $returnSalesOrder->salesOrder->reference_no }}</p>
                        </div>
                        <div>
                            <h4 class="text-xs uppercase font-medium text-slate-600 dark:text-slate-300">returnSalesOrder Date:</h4>
                            <p class="text-slate-500 dark:text-slate-300">{{ Carbon\Carbon::parse($returnSalesOrder->created_at)->translatedFormat('d F Y') }}</p>
                        </div>
        ...
        <!-- returnSalesOrder Products Table -->
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                            @foreach($returnSalesOrder->productReturnSalesOrder as $product)
                            <tr>
                            ...
                            <tr class="font-bold">
                                ...
                                <td class="px-4 py-3 text-right text-slate-900 dark:text-slate-300">
                                    {{ number_format($returnSalesOrder->productreturnSalesOrder->sum('total'), 0, ',', '.') }}
                                </td>
                            </tr>
                        </tbody>
                    </table>

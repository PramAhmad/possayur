...
class SalesOrderExportController extends Controller
{
    ...
    $salesOrder = SalesOrder::with('outlet', 'customer', 'products', 'products.product',
    'returnSalesOrder', 'invoice', 'products.variant',
    'products.batch', 'products.product.unit')->find($id);

    $spreadsheet = new Spreadsheet();
    $sheet = $spreadsheet->getActiveSheet();

    // Set header (kiri: informasi perusahaan, kanan: invoice)
    $sheet->setCellValue('A1', 'Koyasai')->mergeCells('A1:F1')->getStyle('A1')->getFont()->setBold(true)->setSize(16);
    ...
    $sheet->setCellValue('G1', 'INVOICE')->mergeCells('G1:J1')->getStyle('G1')->getFont()->setBold(true)->setSize(16);
    ...
    $sheet->setCellValue('B10', 'PRODUCT')->getStyle('B10')->getFont()->setBold(true)->setSize(12);
    // Merge the product header cells
    $sheet->mergeCells('B10:D10');
    $sheet->setCellValue('E10', 'QTY')->getStyle('E10')->getFont()->setBold(true)->setSize(12);
    $sheet->setCellValue('F10', 'UNIT')->getStyle('F10')->getFont()->setBold(true)->setSize(12);
    $sheet->setCellValue('G10', 'UNIT PRICE')->getStyle('G10')->getFont()->setBold(true)->setSize(12);
    $sheet->mergeCells('G10:H10');
    $sheet->getStyle('G10')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER);
    
    $sheet->setCellValue('I10', 'TOTAL')->getStyle('I10')->getFont()->setBold(true)->setSize(12);
    ...
    foreach ($salesOrder->products as $product) {
        ...
        // Unit
        $sheet->setCellValue('F' . $row, $product->product->unit->name)->getStyle('F' . $row)->getFont()->setSize(12);
